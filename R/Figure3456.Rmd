...
sel2 <- selmodel(res, type="negexp",   prec="sei", scaleprec=FALSE)
sel3 <- selmodel(res, type="logistic", prec="sei", scaleprec=FALSE)
sel4 <- selmodel(res, type="power",    prec="sei",  scaleprec=FALSE)
sel1b <- selmodel(model, type="halfnorm")
sel2b <- selmodel(res, type="negexp")
sel3b <- selmodel(res, type="logistic")
sel4b <- selmodel(res, type="power")
...
par(mar=c(4,4,0.1,0) + 0.1)
plot(sel1)
plot(sel4, add=TRUE, col="red")
plot(sel1b, add=TRUE, lty = "dotted")
plot(sel4b, add=TRUE, col="red", lty = "dotted")
legend(x = 0.6, y = 1, legend = c("half-normal", "","power"), lty =c(1, 0, 1), col = c("black", "", "red"), bty = "n")
pB <- recordPlot()
invisible(dev.off())
...
lnRR<- 0.3 + beta1*x1j[pos] + beta2*x2i + rnorm(n.study, 0, sqrt(sigma2.s))[pos] + 
        rnorm(n.effect, 0, sqrt(sigma2.u)) +  mvrnorm(1, rep(0, n.effect), sigma)
...
funnel(mod1, col =inferno(25)[pos],
      ylim = c(0, 0.26), xlim = c(-0.6, 0.8),
        ylab = "Stanard error (SE)", xlab = "Effect size (Zr)")
...
funnel (residm, vi = vi_residm, yaxis = "sei",  col = inferno(25)[pos],  
        ylim = c(0, 0.26), xlim = c(-0.6, 0.8),
        ylab = "Stanard error (SE)", xlab = "Residuals marginal (lnRR)")
...
funnel(residc1, vi = vi_residc1, yaxis = "sei",  col = inferno(25)[pos],
       ylim = c(0, 0.26), xlim = c(-0.6, 0.8),
        ylab = "Stanard error (SE)", xlab = "Residuals conditional 1 (lnRR)")

# Panel D
funnel(residc2, vi = vi_residc2, yaxis = "sei",  col = inferno(25)[pos],
       ylim = c(0, 0.26), xlim = c(-0.6, 0.8),
        ylab = "Stanard error (SE)", xlab = "Residuals conditional 2 (lnRR)")
