...
legend(x = 1, y = 0, legend = c("0.05 < p < 0.10", ""), pch = c(15, 0), col = c("gray55", "white"), bg = "white")
d <- recordPlot()
invisible(dev.off())

# e
e <- viz_sunset(dat[,c("yi", "sei")], 
...
fig3 <- (ggdraw(a) + ggdraw(b)) /(ggdraw(c) + ggdraw(d))/ (plot_grid(e) + ggdraw(f))/ (ggdraw(g) + ggdraw(h)) + plot_annotation(tag_levels = 'a')
