...
#link-citations: yes
nocite: | 
  @viechtbauer2010conducting, @nakagawa2012methodological, @senior2016heterogeneity, @michonneau2016rotl, @paradis2019ape, @nakagawa2019orchard,  @senior2020revisiting, @noble2017nonindependence, @garamszegi2012meta, @cinar2020phylogenetic, @chamberlain2012does, @hinchliff2015synthesis, @rees2017automated, @begg1994operating, @duval2000trim, @duval2000nonparametric, @stanley2014meta, @peters2007performance, @marks2020historical, @citkowicz2017parsimonious, @murphy2014meta, @odea2021prisma, @wang_using_1998, @hedges_estimation_1984, @rosenthal_file_1979, @begg_operating_1994, @iyengar_selection_1988, @rosenberg_file-drawer_2005, @copas_inference_1997, @orwin_fail-safe_1983, @vivea_publication_2019, @sutton_publication_2009 , @sutton_publication_2009, @knapp2003improved, stanley2017limitations
---
...
# @TO-DO: package ‘klippy’ is not available (for R version 4.0.2)
...
$$
\frac{y_{i}}{se_i} = \beta_{0} + \beta_{1}\left(\frac{1}{se_{i}}\right) + e_i,\\
...
```{r, fig.height = 9, fig.width = 9.5, fig.align = "center", fig.cap="Phylogenetic tree of all species included in the meta-analytic dataset. Notice that some of the names shown in the tree correspond to the most updated synonyms according to the Open Tree Taxonomy (Rees and Cranston 2017) of those originally avialable in the dataset of Garamszegi et al. (2012)."}
...
```{r, fig.cap="Funnel plots with SE (left-hand side) and precision (1/SE; right-hand side) as measures of uncertainty to explore the existence of outliers in the meta-analytic dataset."}
par(mfrow = c(1, 2))
...
```{r, fig.cap="Funnel plot, using precision (1/SE) as a measure of uncertainty, showing the meta-analytic mean obtained in the phylogenetic multilevel intercept-only meta-analysis as a dotted vertical line."}
...
In a simpler meta-regression model (only with the moderator 'SE'), the best estimate for the adjusted overall effect size would be the *intercept* of that model, we the intercept is not significant (i.e. Stanley and Doucouliagos 2012, 2014; more in the main text and also see our corrigendum). Note that Stanley (2017) suggests to use the significance level of 10% to assess the intercept's statistical significance. In a complex model with a categorical variables, it is difficult to decide what is the intercept. Nonetheless, the models above suggests that there are indications of some effects at least in some category. Therefore, in the all-in publication bias test, our approach to estimate the adjusted overall effect is to fit a similar multi-moderator regression to the one above but including the effect sizes' sampling variances (SV or vi) instead of their standard errors as a moderator. This is because when there is an (true) overall effect, the use of 'SV' leads to an adjusted overall effect that is less downward biased than when using 'SE' (more in the main text; Stanley and Doucouliagos 2012, 2014). Note that there is weak evidence for an overall effects, fitting 'SE' gives the best estimate of adjusted effects.
...
mm.publication.bias.model.r.v.1 <- emmeans(res.publication.bias.model.r.v.1, specs = "CaptivityC", df = 104 - 7)
...
The full model provided an adjusted overall effect size that was slightly smaller (adjusted effect = `r round(summary(overall.res.publication.bias.model.r.v.1)[,2],2)`, 95% CI = [`r round(summary(overall.res.publication.bias.model.r.v.1)[,3],2)`,`r round(summary(overall.res.publication.bias.model.r.v.1)[,4],2)`]) than that obtained by the multilevel meta-analysis (unadjusted effect = `r round(metaanalytic.mean.model.r[[1]],2)`, 95% CI = [`r round(metaanalytic.mean.model.r[[3]],2)`,`r round(metaanalytic.mean.model.r[[4]],2)`]; more in section S4.1.3), highlighting the downward correction of the overall effect after accounting for publication bias. The full model also showed slightly smaller effects for all levels of the categorical factor 'captivity status' than the unadjusted model (Table S4.2).
...
```{r, fig.width=7, fig.height= 8, fig.cap="Funnel plots with 1/SE for lnRR (left-hand side) and SMD (right-hand side) as measures of uncertainty to visualise outliers in the meta-analytic dataset."}
...
```{r, fig.width=7, fig.height= 8, fig.cap="Funnel plots with effective sample size as the measure of uncertainty for lnRR (left-hand side) and SMD (right-hand side)."}
# removing outliers
...
Effect sizes with larger uncertainty [larger sqrt(inv_n\_tilda)] tend to be larger (i.e. small-study effects) as indicated because the statistically significant slope of the moderator 'sqrt_inv_n\_tilda' for lnRR (slope = `r round(estimates.publication.bias.model.rr.srin[2,2],2)`, 95% CI = [`r round(estimates.publication.bias.model.rr.srin[2,3],2)`,`r round(estimates.publication.bias.model.rr.srin[2,4],2)`]; *R*<sup>2</sup><sub>marginal</sub> = `r round(orchaRd::r2_ml(publication.bias.model.rr.srin)[[1]]*100,1)`%) and SMD (slope = `r round(estimates.publication.bias.model.smd.srin[2,2],2)`, 95% CI = [`r round(estimates.publication.bias.model.smd.srin[2,3],2)`,`r round(estimates.publication.bias.model.smd.srin[2,4],2)`]; *R*<sup>2</sup><sub>marginal</sub> = `r round(orchaRd::r2_ml(publication.bias.model.smd.srin)[[1]]*100,1)`%). In S4.2.3.3, we will use all-in publication bias test (multi-moderator meta-regression)see whether the small-study effects exist after accounting for some of the heterogeneity present in the data.
...
According to this uni-moderator meta-regression, there is an indication of decline effects since the slope of the moderator 'year of publication' is close tp statistical significance for lnRR (slope = `r round(estimates.publication.bias.model.rr.timelag[2,2],2)`, 95% CI = [`r round(estimates.publication.bias.model.rr.timelag[2,3],2)`,`r round(estimates.publication.bias.model.rr.timelag[2,4],2)`]; *R*<sup>2</sup><sub>marginal</sub> = `r round(orchaRd::r2_ml(publication.bias.model.rr.timelag)[[1]]*100,1)`%) and SMD (slope = `r round(estimates.publication.bias.model.smd.timelag[2,2],2)`, 95% CI = [`r round(estimates.publication.bias.model.smd.timelag[2,3],2)`,`r round(estimates.publication.bias.model.smd.timelag[2,4],2)`]; *R*<sup>2</sup><sub>marginal</sub> = `r round(orchaRd::r2_ml(publication.bias.model.smd.timelag)[[1]]*100,1)`%).
...
First, the multi-moderator meta-regression show a statistically significant slope for the moderator 'sqrt_inv_n\_tilda' (slope for lnRR = `r round(estimates.publication.bias.model.rr.all.srin[1,2],2)`, 95% CI = [`r round(estimates.publication.bias.model.rr.all.srin[1,3],2)`,`r round(estimates.publication.bias.model.rr.all.srin[1,4],2)`]; Figure S4.9) and (slope for SMD = `r round(estimates.publication.bias.model.smd.all.srin[1,2],2)`, 95% CI = [`r round(estimates.publication.bias.model.smd.all.srin[1,3],2)`,`r round(estimates.publication.bias.model.smd.all.srin[1,4],2)`]; Figure S4.10), confirming evidence of small-study effects. Note that Murphy et al. (2013) originally reported no funnel asymmetry from visual inspection, however, we do not recommend using this method for meta-analytic datasets with multiple levels of non-independence and high heterogeneity.

Second, the all-in publication bias test show no evidence of decline effects for lnRR since the moderator 'year of publication' is indistinguishable from zero (slope = `r round(estimates.publication.bias.model.rr.all.srin[2,2],2)`, 95% CI = [`r round(estimates.publication.bias.model.rr.all.srin[2,3],2)`,`r round(estimates.publication.bias.model.rr.all.srin[2,4],2)`]; Figure S4.12). However, the effect size SMD tend to show evidence of decline effects, as the slope of publication year is close to statistical significance (slope = `r round(estimates.publication.bias.model.smd.all.srin[2,2],2)`, 95% CI = [`r round(estimates.publication.bias.model.smd.all.srin[2,3],2)`,`r round(estimates.publication.bias.model.smd.all.srin[2,4],2)`]; Figure S4.13).

##### Figure S4.9

```{r fig.height = 5.5, fig.width = 7.5, fig.align = "center", fig.cap="More negative estimates of lnRR are less certain (larger square root of inverse of effective sample size), showing evidence of small-study effects in the meta-analytic dataset. The solid line represents the model estimate and the shading shows its 95% confidence intervals."}
...
```{r fig.height = 5.5, fig.width = 7.5, fig.align = "center", fog.caption = "More negative estimates of SMD have smaller effective sample sizes (larger square root of inverse of effective sample size), showing evidence of small-study effects in the meta-analytic dataset. The solid line represents the model estimate and the shading shows its 95% confidence intervals."}
# predicting effect sizes for a range of SE from the minimum observed in the data to the maximum observed in data, while year is kept at its mean value, in this case 0 since year was mean-centred
#conditioned on observational studies
...
```{r fig.height = 5.5, fig.width = 7.5, fig.align = "center", fig.cap="The average effect, as measured by lnRR, did not statistically significantly change with time. The solid line represents the model estimate, the shading shows its 95% confidence intervals."}
# predicting effect sizes for a range of sqrt(inv_n_tilda) from the minimum observed in the data to the maximum observed in data, while year is kept at its mean value, in this case 0 since year was mean-centred
#conditioned on observational studies
...
```{r fig.height = 5.5, fig.width = 7.5, fig.align = "center", fig.cap="The average effect, as measured by SMD, has declined, although non-signficant, over the timespan captured by this dataset. The solid line represents the model estimate, the shading shows its 95% confidence intervals."}
# predicting effect sizes for a range of sqrt(inv_n_tilda) from the minimum observed in the data to the maximum observed in data, while year is kept at its mean value, in this case 0 since year was mean-centred
...
The models above do not provide strong evidence that there are a significant overall effect (or a significant intercept or at least, one group being significant at the alpha level at 10%, which translates to a t value of > ~1.65 in magnitude). Therefore, we here fit $\sqrt(1/\tilde{n})$ rather than $1/\tilde{n}$, which should be used when there is evidence an overall effect exists or some groups have non-zero effects.
...
overall.res.publication.bias.model.rr.v.1 <- emmeans(res.publication.bias.model.rr.v.1, specs = ~1, df = publication.bias.model.rr.v.1$ddfs, weights = "prop") # using effect size - 7 
# marginalized means for different levels for Experimental.or.Observational.
mm.publication.bias.model.rr.v.1 <- emmeans(res.publication.bias.model.rr.v.1, specs = "Experimental.or.Observational.", df = publication.bias.model.rr.v.1$ddfs, weights = "prop")
...
# marginalized overall mean at inv_n_tilda = 0 and year.c = 0; also weights = "prop" or "cells" average things over proportionally. if not specified, all groups (levels) get the same weights
overall.res.publication.bias.model.smd.v.1 <- emmeans(res.publication.bias.model.smd.v.1, specs = ~1, df = publication.bias.model.smd.v.1$ddfs, weights = "prop") # using effect size - 7 
# marginalized means for different levels for Experimental.or.Observational.
mm.publication.bias.model.smd.v.1 <- emmeans(res.publication.bias.model.smd.v.1, specs = "Experimental.or.Observational.", df = publication.bias.model.smd.v.1$ddfs, weights = "prop")
...
The full model provided an adjusted overall lnRR that was obviously smaller (adjusted effect = `r round(summary(overall.res.publication.bias.model.rr.v.1)[,2],2)`, 95% CI = [`r round(summary(overall.res.publication.bias.model.rr.v.1)[,3],2)`,`r round(summary(overall.res.publication.bias.model.rr.v.1)[,4],2)`]) than that obtained by the multilevel meta-analysis (unadjusted effect = `r round(metaanalytic.mean.model.rr[[1]],2)`, 95% CI = [`r round(metaanalytic.mean.model.rr[[3]],2)`,`r round(metaanalytic.mean.model.rr[[4]],2)`]; more in section S4.2.2), highlighting the downward correction of the overall effect after accounting for publication bias and time-lag bias. The full model also showed slightly smaller effects for all levels of the categorical factor 'Experimental.or.Observational.' than that of the unadjusted model (Table S4.3).

```{r}
# creating a table to show the results of model 2
table.comparing.Exp.or.Obs.levels <- merge(estimates.publication.bias.model.rr.v.1,
                                          estimates.publication.bias.model.rr.v.1b,
                                          by="estimate",
                                          all.x=T)
# rounding estimates
table.comparing.Exp.or.Obs.levels <- table.comparing.Exp.or.Obs.levels %>% mutate_at(vars(mean.x, lower.x,upper.x,mean.y,lower.y,upper.y), list(~round(., 2)))
...
**Table S4.3**. The estimates of all levels of the moderator 'Experimental.or.Observational.' and their adjusted estimates using the all-in publication bias test (multi-moderator meta-regression). Estimates are presented as lnRR.

SMD also show the downward correction of the overall effect after accounting for publication bias and time-lag bias: (adjusted effect = `r round(summary(overall.res.publication.bias.model.smd.v.1)[,2],2)`, 95% CI = [`r round(summary(overall.res.publication.bias.model.smd.v.1)[,3],2)`,`r round(summary(overall.res.publication.bias.model.smd.v.1)[,4],2)`]) and (unadjusted effect = `r round(metaanalytic.mean.model.smd[[1]],2)`, 95% CI = [`r round(metaanalytic.mean.model.smd[[3]],2)`,`r round(metaanalytic.mean.model.smd[[4]],2)`]; more in section S4.2.2). Table S4.4 also showed slightly smaller effects for all levels of the categorical factor 'Experimental.or.Observational.' compared to unadjusted model.

```{r}
# creating a table to show the results of model 2
table.comparing.Exp.or.Obs.levels <- merge(estimates.publication.bias.model.smd.v.1,
...
**Table S4.4**. The estimates of all levels of the moderator 'Experimental.or.Observational.' and their adjusted estimates using the all-in publication bias test (multi-moderator meta-regression). Estimates are presented as SMD.
...
```{r, dpi = 10, fig.align = "center", fig.cap="Figure 6a-d redrawn"}
knitr::include_graphics(here('figs', 'fig6_extra.png'))
```
...
The trim-and-fill method identified `r trimfill.r$k0` (SE = `r round(trimfill.r$se.k0,2)`) missing studies (Figure S5.1) and estimated an adjusted overall effect of `r round(trimfill.r$beta[[1]],2)` (95% CI = [`r round(trimfill.r$ci.lb[[1]],2)`,`r round(trimfill.r$ci.ub[[1]],2)`]), which is similar to the adjusted overall effect estimated by our suggested multilevel meta-regression method (`r round(estimates.publication.bias.model.r.v.1[1,2],2)`, 95% CI = [`r round(estimates.publication.bias.model.r.v.1[1,3],2)`,`r round(estimates.publication.bias.model.r.v.1[1,4],2)`]; more in section S4.1.4.3).

<br/><br/>

##### Figure S5.1

```{r fig.align = "center", fig.cap="Funnel plot using precision (1/SE) as a measure of uncertainty and showing the adjusted meta-analytic mean (vertical line) and the  missing effect sizes (white points) according to the trim-and-fill method."}
...
```{r fig.align = "center", fig.cap="Results of a step function selection model based on one cut-point ($\alpha = 0.05$) for a model without any moderators (left-hand figure) and a model including both 'year of publication' and 'captivity status' as moderators (right-hand figure)."}
par(mfrow = c(1, 2))
...
```{r fig.align = "center", fig.cap="Forest plot showing the results of a cumulative meta-analysis, showing no clear evidence of decline effects."}
...
The median number of missing studies was `r median(trimfill.sampling.r$n)` (range = `r min(trimfill.sampling.r$n)` - `r max(trimfill.sampling.r$n)`; Figure S5.4A), which is seemingly lower than the number of missing studies estimated using the aggregated data (n = `r trimfill.r$k0`; more in section S5.1.1.1). Additionally, the median adjusted overall effect according to the trim-and-fill method was `r round(median(trimfill.sampling.r$beta),2)` (range = `r round(min(trimfill.sampling.r$beta),2)` - `r round(max(trimfill.sampling.r$beta),2)`; Figure S5.4B), which is very similar to the adjusted overall effect estimated using the aggregated data (`r round(trimfill.r$b[[1]],2)`; more in section S5.1.1.1), but still lower than the adjusted overall effect estimated by our suggested multilevel meta-regression method (`r round(estimates.publication.bias.model.r.v.1[1,2],2)`, 95% CI = [`r round(estimates.publication.bias.model.r.v.1[1,3],2)`,`r round(estimates.publication.bias.model.r.v.1[1,4],2)`]; more in section S4.1.4.3).

<br/><br/>

##### Figure S5.4

```{r fig.align = "center", fig.cap="Distribution of 1000 (A) estimated number of missing studies according to the trim-and-fill method, and (B) adjusted overall effect sizes according to the trim-and-fill method. The dashed line shows the median value."}
...
The median adjusted overall effect according to the 3 parameter selection model was `r round(median(threePSM.sampling.r$beta),2)` (range = `r round(min(threePSM.sampling.r$beta),2)` - `r round(max(threePSM.sampling.r$beta),2)`; Figure S5.4B), which is similar to the adjusted overall effect estimated using the aggregated data (`r round(three.PSM.r0$beta[[1]],2)`; 95% CI = [`r round(three.PSM.r0$ci.lb[[1]],2)`,`r round(three.PSM.r0$ci.ub[[1]],2)`]; more in section S5.1.1.2) and to the adjusted overall effect estimated by our suggested multilevel meta-regession method (`r round(estimates.publication.bias.model.r.v.1[1,2],2)`, 95% CI = [`r round(estimates.publication.bias.model.r.v.1[1,3],2)`,`r round(estimates.publication.bias.model.r.v.1[1,4],2)`]; more in section S4.1.4.3).

<br/><br/>

##### Figure S5.5

```{r fig.align = "center", fig.cap="Distribution of 1000 adjusted overall effect sizes according to the 3 parameter selection model. The dashed line shows the median value."}
...
```{r fig.align = "center", fig.cap="Median and 95% quantiles of 1000 effect sizes estimated according to cumulative meta-analysis showing no evidence of decline effects."}
...
The step function selection model based on one cut-point ($\alpha = 0.05$) for the intercept-only model estimated an adjusted overall lnRR of `r round(three.PSM.rr0$beta[[1]],2)` (95% CI = [`r round(three.PSM.rr0$ci.lb[[1]],2)`,`r round(three.PSM.rr0$ci.ub[[1]],2)`]; Figure S5.8), which is smaller than the original overall effect estimated by the meta-analytic model (`r round(metaanalytic.mean.model.r[[1]],2)`, 95% CI = [`r round(metaanalytic.mean.model.r[[3]],2)`,`r round(metaanalytic.mean.model.r[[4]],2)`]; more in section S4.2.3.3). SMD showed the same pattern. Overall, the selection model estimated adjusted overall effects that were either very similar or even slightly larger than the original unadjusted effect sizes (Table S5.2), which might indicate that the specific selection model that we used (i.e. one cut-point of $\alpha = 0.05$) was not an adequate approximation for the underlying selection process.
...
```{r fig.align = "center", fig.cap="Distribution of `r n.randomization.rr.TAF` (A) estimated number of missing studies according to the trim-and-fill method, and (B) adjusted overall lnRR according to the trim-and-fill method. The dashed line shows the median value."}
# plotting the distribution of the number of missing studies according to the trim-and-fill method
...
The median number of missing studies for SMD was `r median(trimfill.sampling.smd$n)` (range = `r min(trimfill.sampling.smd$n)` - `r max(trimfill.sampling.smd$n)`; Figure S5.10A). Additionally, the median adjusted overall SMD according to the trim-and-fill method was `r round(median(trimfill.sampling.smd$beta),2)` (range = `r round(min(trimfill.sampling.smd$beta),2)` - `r round(max(trimfill.sampling.smd$beta),2)`; Figure S5.10B), which is larger than the adjusted overall SMD estimated by our suggested multilevel meta-regression method (`r round(estimates.publication.bias.model.smd.v.1[1,2],2)`, 95% CI = [`r round(estimates.publication.bias.model.smd.v.1[1,3],2)`,`r round(estimates.publication.bias.model.smd.v.1[1,4],2)`]; more in section S4.2.4.3).

<br/><br/>

##### Figure S5.10

```{r fig.align = "center", fig.cap="Distribution of `r n.randomization.smd.TAF` (A) estimated number of missing studies according to the trim-and-fill method, and (B) adjusted overall SMD according to the trim-and-fill method. The dashed line shows the median value."}
# plotting the distribution of the number of missing studies according to the trim-and-fill method
...
The median adjusted overall lnRR according to the 3 parameter selection model was `r round(median(threePSM.sampling.rr$beta),2)` (range = `r round(min(threePSM.sampling.rr$beta),2)` - `r round(max(threePSM.sampling.rr$beta),2)`; Figure S5.4B), which is smaller than the adjusted overall lnRR estimated by our suggested multilevel meta-regression method (`r round(estimates.publication.bias.model.rr.v.1[1,2],2)`, 95% CI = [`r round(estimates.publication.bias.model.rr.v.1[1,3],2)`,`r round(estimates.publication.bias.model.rr.v.1[1,4],2)`]; more in section S4.2.4.3).

<br/><br/>

##### Figure S5.11

```{r fig.align = "center", fig.cap="Distribution of `r n.randomization.rr.3PSM` adjusted overall lnRR according to the 3 parameter selection model. The dashed line shows the median value."}
# plotting the distribution of adjusted overall effect sizes according to the 3 parameter selection model
...
```{r fig.align = "center", fig.cap="Distribution of `r n.randomization.smd.3PSM` adjusted overall SMD according to the 3 parameter selection model. The dashed line shows the median value."}
# plotting the distribution of adjusted overall effect sizes according to the 3 parameter selection model
...
```{r fig.align = "center", fig.cap="Median and 95% quantiles of `r n.randomization.rr.CM` lnRR estimated according to cumulative meta-analysis showing evidence of decline effects."}
...
```{r fig.align = "center", fig.cap="Median and 95% quantiles of `r n.randomization.smd.CM` SMD estimated according to cumulative meta-analysis showing evidence of decline effects."}
