...
SMD also show similar patterns for the adjusted overall effects (Adjusted 1 with $\sqrt(1/\tilde{n})$  and Adjusted 2 with $1/\tilde{n}$ ) after accounting for publication bias and time-lag bias: (Adjusted 2 = `r round(summary(overall.res.publication.bias.model.smd.v.0)[,2],2)`, 95% CI = [`r round(summary(overall.res.publication.bias.model.smd.v.0)[,3],2)`,`r round(summary(overall.res.publication.bias.model.smd.v.0)[,4],2)`]; Adjusted 1 with $\sqrt(1/\tilde{n})$  and Adjusted 2 with $1/\tilde{n}$ ) after accounting for publication bias and time-lag bias: (Ajusted 2 = `r round(summary(overall.res.publication.bias.model.smd.v.1)[,2],2)`, 95% CI = [`r round(summary(overall.res.publication.bias.model.smd.v.1)[,3],2)`,`r round(summary(overall.res.publication.bias.model.smd.v.1)[,4],2)`]; unadjusted effect = `r round(metaanalytic.mean.model.smd[[1]],2)`, 95% CI = [`r round(metaanalytic.mean.model.smd[[3]],2)`,`r round(metaanalytic.mean.model.smd[[4]],2)`]; more in section S4.2.2). As with lnRR, Table S4.4 show that the estimates from the unadjusted model and the adjusted model with  $\sqrt(1/\tilde{n})$ while the adjusted model with  $\sqrt(1/\tilde{n})$ gave smaller estimates in magnitude. Therefore, for SMD, we also suggest that we use the estimates from the method with $1/\tilde{n}$ (Adjusted mean 2 and Adjusted 95% CI 2)
