...
By checking the dataset, We found the minimum lnRR and SMD were unsual: the minimum lnRR is -2.79 and SMD -11.3. We decided to taking the minimum values out. Then we checkd the funnel plots agian (Figure S4.7). This time, funnel plots seemd to be normal.
...
We found that there was a indication of small-study effects since the slopes of the moderator 'sqrt(inv_n_tilda)' is marginally statistically significant for lnRR (slope = `r round(estimates.publication.bias.model.rr.srin[2,2],2)`, 95% CI = [`r round(estimates.publication.bias.model.rr.srin[2,3],2)`,`r round(estimates.publication.bias.model.rr.srin[2,4],2)`]; *R<sup>2</sup><sub>marginal</sub>* = `r round(orchaRd::r2_ml(estimates.publication.bias.model.rr.srin)[[1]]*100,1)`%) and statistically significant for SMD (slope = `r round(estimates.publication.bias.model.smd.srin[2,2],2)`, 95% CI = [`r round(estimates.publication.bias.model.smd.srin[2,3],2)`,`r round(estimates.publication.bias.model.smd.srin[2,4],2)`]; *R<sup>2</sup><sub>marginal</sub>* = `r round(orchaRd::r2_ml(estimates.publication.bias.model.smd.srin)[[1]]*100,1)`%), which showed that effect sizes with larger uncertainty [larger sqrt(inv_n_tilda)] tend to be larger. Nonetheless, as mentioned above, we will confirm whether the small-study effects exist after accounting for some of the heterogeneity present in the data using the all-in publication bias test (multi-moderator meta-regression) described in section S4.2.4.3.
...
newdat <- data.frame(sei= dataset.rr$sqrt_inv_n_tilda,
...
newdat <- data.frame(sei= dataset.smd$sqrt_inv_n_tilda,
...
table.model.rr.Exp.or.Obs.gt.3PSM <- table.comparing.Exp.or.Obs.levels.3PSM %>% 
  gt() %>% 
  cols_label(captivity.level=md("**Experimental.or.Observational.**"),
             adjusted.mean=md("**Adjusted mean**"),
             adjusted.CI=md("**Adjusted 95% CI**"),
             unadjusted.mean=md("**Unadjusted mean**"),
             unadjusted.CI=md("**Unadjusted 95% CI**")) %>%
...
n.randomization.rr.TAF <- 1000 #number of samplings to be done
 ...
 save(trimfill.sampling.rr,file = here("data","trimfill_sampling_rr_1000s.rrData"))
...
n.randomization.smd.TAF <- 1000 #number of samplings to be done
 ...
 save(trimfill.sampling.smd,file = here("data","trimfill_sampling_smd_1000s.smdData"))
...
# save(cumul.sampling.rr,file = here("data","cumul_sampling_r_1000s.rrData"))
...
load(here("data","cumul_sampling_rr_1000s.rrData"))
...
# save(cumul.sampling.smd,file = here("data","cumul_sampling_r_1000s.smdData"))
...
load(here("data","cumul_sampling_smd_1000s.smdData"))
```
