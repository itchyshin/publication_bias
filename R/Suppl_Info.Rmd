...
# centring year of publication to help with interpretation, particularly in S4.1.4.3.
...
In this case, the all-in publication bias test confirms what we observed in the uni-moderator meta-regressions above (sections S4.1.4.1 and S4.1.4.2). First, the multi-moderator meta-regression shows a statistically significant slope for the moderator 'SE' (**slope = `r round(estimates.publication.bias.model.r.all.se[1,2],2)`, 95% CI = [`r round(estimates.publication.bias.model.r.all.se[1,3],2)`,`r round(estimates.publication.bias.model.r.all.se[1,4],2)`]**; Figure S4.4), showing evidence of a small-study effect. In other words, the largest effect sizes in the dataset tend to be those with the lowest precision (i.e. larger uncertainty; Figure S4.4). This result agrees with the findings originally reported in Garamszegi et al. (2012), where the authors found evidence of publication bias after applying a simple correlation method first proposed by Begg and Mazumdar (1994) and the trim-and-fill method (Duval and Tweedie 2000) - although bear in mind that we do not recommend to use those simple methods for meta-analytic databases with multiple levels of non-independence and high heterogeneity (more in the main text). Second, the all-in publication bias test also confirms that there is no evidence of decline effects in these data since the slope of the moderator 'year' was again essentially zero (**slope = `r round(estimates.publication.bias.model.r.all.se[2,2],2)`, 95% CI = [`r round(estimates.publication.bias.model.r.all.se[2,3],2)`,`r round(estimates.publication.bias.model.r.all.se[2,4],2)`]**; Figure S4.5). Overall, the moderators included in this multi-moderator meta-regression explained a total of `r round(r2_ml(publication.bias.model.r.all.se)[[1]]*100,1)`% (i.e. *R<sup>2</sup><sub>marginal</sub>*) of the heterogeneity observed in the meta-analytic model (Table S4.1).

```{r}
# to plot the 'sei' effect we are simply including the moderator 'CaptivityC' as a random effect for simplicity of plotting. Note that this does not change the estimates of the moderators.
...
```{r fig.height = 5.5, fig.width = 7.5, fig.align = "center"}
# predicting effect sizes for a range of years from the minimum observed in the data to the maximum observed in data, while SE is kept at its mean value
...
2. Multi-moderator meta-regression including sampling variance, year of publication (mean-centred) and the categorical factor 'captivity status', which has five levels, as moderators. In this meta-regression, we are going to remove the intercept so that we obtained adjusted overall effects for each of the levels of the factor 'captivity status'.

If the goal was to provide an adjusted overall effect size after accouting for the variance explained by 'captivity status', we could also either include 'captivity status', which has five levels, as a random effect or consider mean-centring this categorical factor following Schielzeth (2010). In this example, we will keep it simple and only run the two models discussed above.
...
# model 2: providing an adjusted effect for each level of CaptivityC
...
# model 2b: comparing results without correcting for publication bias
publication.bias.model.r.v.2b <- rma.mv(yi, vi,
                                        mods=~-1+
                                          CaptivityC, # take Class - that is already a part of phylo   
                                        random=list(~ 1 | Species.updated, # phylo effect
                                                    ~ 1 | Species, # non-phylo effect
...
# # model 3: adding CaptivityC as a random effect to provide an adjusted overall effect across levels
...
# # mean-centring the categorical predictor 'CaptivityC' by creating dummy variables as suggested in Schielzeth 2010 (https://doi.org/10.1111/j.2041-210X.2010.00012.x)
# 
# dataset.r$CaptivityC.dummy.1 <- ifelse(dataset.r$CaptivityC=="FW",1,0)
# dataset.r$CaptivityC.dummy.2 <- ifelse(dataset.r$CaptivityC=="WCT",1,0)
# dataset.r$CaptivityC.dummy.3 <- ifelse(dataset.r$CaptivityC=="WCT&FC",1,0)
# dataset.r$CaptivityC.dummy.4 <- ifelse(dataset.r$CaptivityC=="WCT&FW",1,0)
# 
# dataset.r$CaptivityC.dummy.1.c <- scale(dataset.r$CaptivityC.dummy.1,scale=F)
# dataset.r$CaptivityC.dummy.2.c <- scale(dataset.r$CaptivityC.dummy.2,scale=F)
# dataset.r$CaptivityC.dummy.3.c <- scale(dataset.r$CaptivityC.dummy.3,scale=F)
# dataset.r$CaptivityC.dummy.4.c <- scale(dataset.r$CaptivityC.dummy.4,scale=F)
# 
# 
# publication.bias.model.r.v.4 <- rma.mv(yi, vi,
#                                        mods=~1+vi + 
#                                          year.c +
#                                          CaptivityC.dummy.1.c+
#                                          CaptivityC.dummy.2.c+
#                                          CaptivityC.dummy.3.c+
#                                          CaptivityC.dummy.4.c,
#                                        random=list(~ 1 | Species.updated, # phylo effect
#                                                    ~ 1 | Species, # non-phylo effect
#                                                    ~ 1 | study, 
#                                                    ~ 1 | obsID), 
...
# # mean-centring alternative way, not sure about it's validity though
# dataset.r$CaptivityC.dummy <- ifelse(dataset.r$CaptivityC=="FC",0,
#                                      ifelse(dataset.r$CaptivityC=="FW",1,
#                                             ifelse(dataset.r$CaptivityC=="WCT",2,
#                                                    ifelse(dataset.r$CaptivityC=="WCT&FC",3,4))))
# 
# dataset.r$CaptivityC.dummy.c <- as.vector(scale(dataset.r$CaptivityC.dummy,scale=F))
# 
# 
# publication.bias.model.r.v.5 <- rma.mv(yi, vi,
#                                          mods=~1+vi + 
#                                            year.c +
#                                            CaptivityC.dummy.c, # take Class - that is already a part of phylo 
#                                          random=list(~ 1 | Species.updated, # phylo effect
#                                                      ~ 1 | Species, # non-phylo effect
#                                                      ~ 1 | study, 
#                                                      ~ 1 | obsID), 
...
estimates.publication.bias.model.r.v.2b <- estimates.CI(publication.bias.model.r.v.2b)
```

Model 1 estimated an adjusted overall effect size that was slightly smaller (**adjusted effect = `r round(estimates.publication.bias.model.r.v.1[1,2],2)`, 95% CI = [`r round(estimates.publication.bias.model.r.v.1[1,3],2)`,`r round(estimates.publication.bias.model.r.v.1[1,4],2)`]**) than that obtained by the multilevel meta-analysis (**unadjusted effect = `r round(metaanalytic.mean.model.r[[1]],2)`, 95% CI = [`r round(metaanalytic.mean.model.r[[3]],2)`,`r round(metaanalytic.mean.model.r[[4]],2)`]**; more in section S4.1.3), reflecting the correction after accounting for publication bias. Model 2 also showed slightly smaller effects for all levels of the categorical factor 'captivity status' (Table S4.2). 

```{r}
# creating a table to show the results of model 2
table.comparing.captivity.levels <- merge(estimates.publication.bias.model.r.v.2[c(3:7),],
                                          estimates.publication.bias.model.r.v.2b,
                                          by="estimate",
                                          all.x=T)
...
**Table S4.2.** Results adjusted and unadjusted for publication bias using the all-in publication bias test (multi-moderator meta-regression). Estimates are presented as standardized effect sizes using Fisher’s transformation (i.e. **Zr**).
