...
<!--- please label Figs like this - level 5--->
...
Something like it is important to check data before analyse checking outliers (which could be extraction mistakes)
...
#### S4.1.4: Publication bias tests with mulitlevel meta-regression
...
# Application of Equation 21 from the main manuscript m
...
publication.bias.model.r.se.plot <- predict(publication.bias.model.r.se)

newdat <- data.frame(sei=dataset.r.original$sei,
                     fit=publication.bias.model.r.se.plot$pred,
                     upper=publication.bias.model.r.se.plot$ci.ub,
                     lower=publication.bias.model.r.se.plot$ci.lb,
...
```{r fig.height = 5.5, fig.width = 7.5, fig.align = "center"}
publication.bias.model.r.timelag.plot <- predict(publication.bias.model.r.timelag,newmods=seq(min(dataset.r$year),max(dataset.r$year),1))

newdat <- data.frame(year=seq(min(dataset.r$year),max(dataset.r$year),1),
                     fit=publication.bias.model.r.timelag.plot$pred,
                     upper=publication.bias.model.r.timelag.plot$ci.ub,
                     lower=publication.bias.model.r.timelag.plot$ci.lb,
                     stringsAsFactors=FALSE)
...
##### S4.1.4.3: All-in publication bias test (mulit-moderators)
...
# for plotting year effect
publication.bias.model.r.all.se2 <- rma.mv(yi, vi,
                                            mods=~scale(sei)+
                                            year,
                                            random=list(~1 | CaptivityC,
                                              ~ 1 | Species.updated, # phylo effect
...
meta.regression.r.resduals.funnel.plot <- rma.mv(yi, vi,
                                            mods=~1 +
                                            scale(year)+
...
funnel(meta.regression.r.resduals.funnel.plot, yaxis = "seinv")
...
publication.bias.model.r.v <- rma.mv(yi, vi,
                                           mods=~vi + 
                                           scale(year) +
                                           CaptivityC - 1, # take Class - that is already a part of phylo   
                                           random=list(~ 1 | Species.updated, # phylo effect
...
summary(publication.bias.model.r.v)
