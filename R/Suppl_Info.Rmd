...
co# # need pacman to install the rest
...
publication.bias.model.r <- rma.mv(yi, vi,
                                       mods=~ 1 + vi + 
                                         year.c +
...
res.publication.bias.model.r.v.1 <- qdrg(object = publication.bias.model.r, data = dataset.r, at = list(vi = 0, year.c = 0))
emmeans(res.publication.bias.model.r, specs = ~1, df = 104 - 7) # using effect size - 7 
emmeans(res.publication.bias.model.r, specs = "CaptivityC", df = 104 - 7)
#emmeans(res.publication.bias.model.r, specs = "year.c")
#emmeans(res.publication.bias.model.r, specs = "CaptivityC", weights = "cells")
#emmeans(res.publication.bias.model.r, specs = "CaptivityC", weights = "equal")

emmeans(res.publication.bias.model.r, specs = "CaptivityC", at = list(vi = 0, year.c = 0))
...
table.model.r.captivity.gt <- table.comparing.captivity.levels %>% 
  ...
  cols_align(align = "center")
#@DanCOMMENT: Can you provide what models these are all from base don 3 points above within this table? It will be more clear. ALso, why WCT & FC so high relative to others?"
knitr::kable(table.model.r.captivity.gt, caption = "Results for all levels of the moderator 'captivity status', both adjusted and unadjusted for publication bias using the all-in publication bias test (multi-moderator meta-regression). Estimates are presented as standardized effect sizes using Fisher’s transformation (i.e. *Zr*).")
```
...
In the main text we provided a two-step approach to calculate the adjusted overall effect size, that is, the overall effect after we account for publication bias. The rationale is that, when no uncertainty exists (theoretically), we can get the potential true effect from a multiple-moderator multilevel-model (i.e. all-in publication bias test, where the inclusion of moderator variables reduces heterogeneity). Note that, if the slope for the moderator 'sqrt_inv_n_tilda' in the all-in publication bias test shown above was not statistically significant, then the best estimate for the adjusted overall effect size would be the *intercept* of that all-in publication bias test. Otherwise, we should use inv_n_tilda instead as moderator (detailed in the main text). 

Determining what moderators to include in the multilevel-model will depend on what we want the adjusted overall effect size to reflect. We are going to fit three different models to showcase some of these possibilities. Importantly, we should keep in mind that publication bias tests should be treated as sensitivity analyses to test the robustness of results from the original analysis (Noble et al., 2017). None of the estimated adjusted overall effects should be treated as the real overall value if publication bias did not exist; after all, we can never be sure of what has not been published. 

The three models that we are going to fit are:
<!--- @TO-DO: Shinichi to go through this section. --->
1. Multi-moderator meta-regression only including the inverse of the effective sample size and year of publication (mean-centred) as moderators. The intercept of this meta-regression will represent the adjusted overall effect size. 
...
<!--- @TO-DO: problem in the below chunk when knitting to html (`object obsID not found`), not sure why because the chunks run fine one-by-one.--->
...
#@TO-DO: Shinichi to change this; DN, OK. Will leave...
