...
# custom function for extracting mean and CI for emmeans (marginalised means) 
...
```{r, fig.height = 9, fig.width = 9.5, fig.align = "center", fig.caption="Phylogenetic tree of all species included in the meta-analytic dataset. Notice that some of the names shown in the tree correspond to the most updated synonyms according to the Open Tree Taxonomy (Rees and Cranston 2017) of those originally avialable in the dataset of Garamszegi et al. (2012)."}
...
```{r, fig.caption="Funnel plots with SE (left-hand side) and precision (1/SE; right-hand side) as measures of uncertainty to explore the existence of outliers in the meta-analytic dataset."}
par(mfrow = c(1, 2))
...
```{r, fig.caption="Funnel plot, using precision (1/SE) as a measure of uncertainty, showing the meta-analytic mean obtained in the phylogenetic multilevel intercept-only meta-analysis as a dotted vertical line."}
...
**Table S4.2.** Results for all levels of the moderator 'captivity status', both adjusted and unadjusted for publication bias using the all-in publication bias test (multi-moderator meta-regression). Estimates are presented as standardized effect sizes using Fisher’s transformation (i.e. *Zr*).
...
Funnel plot using effective sample size as a measure of uncertainty, showing the meta-analytic mean obtained in the multilevel intercept-only meta-analysis as dashed vertical lines. <!--- @DanCOMMENT: Don't we have this figure already???---> 
...
To test for publication bias (evidence of small-study effects), we first fitted a uni-moderator multilevel meta-regression including the square root of the inverse of effective sample size $\sqrt{\frac{1}{\tilde{n}}}$ (Equation 26) as the only moderator (see Equation 27 from the main text). This meta-regression will provide some information about the existence of small-study effects (i.e. asymmetry in the distribution of effect sizes) after accounting for non-independence. However, high levels of heterogeneity also create problems for publication bias tests in meta-analytic datasets in ecology and evolution. Therefore, the best evidence approach is to model both heterogeneity and non-independence, by using a multi-moderator multilevel meta-regression model that includes all the important moderators (see all-in publication bias test described in section S4.2.4.3).
...
# @DanCOMMENT: Question for Shin. I understand the inverse, but what happened to 4* in the equation? Not clear why this should "go away". I know you say reduces to 2n when n1=n2, but this won't always be the case. So, should we stick with 4*? More of a clarification.
...
```{r fig.height = 5.5, fig.width = 7.5, fig.align = "center", fig.caption="The average effect, as measured by lnRR, did not statistically significantly change with time. The solid line represents the model estimate, the shading shows its 95% confidence intervals."}
# predicting effect sizes for a range of sqrt(inv_n_tilda) from the minimum observed in the data to the maximum observed in data, while year is kept at its mean value, in this case 0 since year was mean-centred
#conditioned on observational studies
...
```{r fig.height = 5.5, fig.width = 7.5, fig.align = "center", fig.caption="The average effect, as measured by SMD, has marginally declined over the timespan captured by this dataset. The solid line represents the model estimate, the shading shows its 95% confidence intervals."}
# predicting effect sizes for a range of sqrt(inv_n_tilda) from the minimum observed in the data to the maximum observed in data, while year is kept at its mean value, in this case 0 since year was mean-centred
...
In the main text we provided a two-step approach to calculate the adjusted overall effect size, that is, the overall effect after we account for publication bias. The rationale is that, when no uncertainty exists (theoretically), we can get the potential true effect from a multiple-moderator multilevel-model (i.e. all-in publication bias test, where the inclusion of moderator variables reduces heterogeneity). We add two more moderators `Experimental.or.Observational.` (2 levels) and `Disturbance_category` (10 levels; we refer to this model as the full model). Note that, if the slope for the moderator 'sqrt_inv_n_tilda' in the all-in publication bias test shown above was not statistically significant, then the best estimate for the adjusted overall effect size would be the *intercept* of that all-in publication bias test. Otherwise, we should use inv_n_tilda instead as moderator (detailed in the main text). 
...
Model 1 estimated an adjusted overall SMD that was obviously smaller (adjusted effect = `r round(summary(overall.res.publication.bias.model.smd.v.1)[,2],2)`, 95% CI = [`r round(summary(overall.res.publication.bias.model.smd.v.1)[,3],2)`,`r round(summary(overall.res.publication.bias.model.smd.v.1)[,4],2)`]) than that obtained by the multilevel meta-analysis (unadjusted effect = `r round(metaanalytic.mean.model.smd[[1]],2)`, 95% CI = [`r round(metaanalytic.mean.model.smd[[3]],2)`,`r round(metaanalytic.mean.model.smd[[4]],2)`]; more in section S4.2.2), highlithing the downward correction of the overall effect after accounting for publication bias and time-lag bias. The full model also showed slightly smaller effects for all levels of the categorical factor 'Experimental.or.Observational.' that the unadjusted model (Table S4.4).
...
```{r fig.align = "center", fig.caption="Funnel plot using precision (1/SE) as a measure of uncertainty and showing the adjusted meta-analytic mean (vertical line) and the  missing effect sizes (white points) according to the trim-and-fill method."}
...
meta.analysis.r.agg0 <- rma(yi, vi, 
                            method="ML", #method needs to set to "ML" rather than "REML" as in all models above @TO-DO: this isn't clear, not all models above use ML
                            test="knha", #knha is meant to be better than Z and t, but it is not implemented for multilevel models @TO-DO: explain why knha is meant to be better, or else delete
                            data=dataset.r)
...
```{r fig.align = "center", fig.caption="Results of a step function selection model based on one cut-point ($\alpha = 0.05$) for a model without any moderators (left-hand figure) and a model including both 'year of publication' and 'captivity status' as moderators (right-hand figure)."}
par(mfrow = c(1, 2))
...
```{r fig.align = "center", fig.caption="Forest plot showing the results of a cumulative meta-analysis, showing no clear evidence of decline effects."}
...
```{r fig.align = "center", fig.caption="Distribution of 1000 (A) estimated number of missing studies according to the trim-and-fill method, and (B) adjusted overall effect sizes according to the trim-and-fill method. The dashed line shows the median value."}
...
<!--- @TO-DO: here and throughout, the numbers from the different methods seem so similar that contrasting them seems misleading? "slightly smaller than...", and "slightly larger than..." but the confidence intervals suggest they're pretty much the same? --->

The median adjusted overall effect according to the 3 parameter selection model was `r round(median(threePSM.sampling.r$beta),2)` (range = `r round(min(threePSM.sampling.r$beta),2)` - `r round(max(threePSM.sampling.r$beta),2)`; Figure S5.4B), which is slightly smaller than the adjusted overall effect estimated using the aggregated data (`r round(three.PSM.r0$beta[[1]],2)`; 95% CI = [`r round(three.PSM.r0$ci.lb[[1]],2)`,`r round(three.PSM.r0$ci.ub[[1]],2)`]; more in section S5.1.1.2), but still slightly larger than the adjusted overall effect estimated by our suggested multilevel meta-regession method (`r round(estimates.publication.bias.model.r.v.1[1,2],2)`, 95% CI = [`r round(estimates.publication.bias.model.r.v.1[1,3],2)`,`r round(estimates.publication.bias.model.r.v.1[1,4],2)`]; more in section S4.1.4.3).
...
```{r fig.align = "center", fig.caption="Distribution of 1000 adjusted overall effect sizes according to the 3 parameter selection model. The dashed line shows the median value."}
...
```{r fig.align = "center", fig.caption="Median and 95% quantiles of 1000 effect sizes estimated according to cumulative meta-analysis showing no evidence of decline effects."}
...
```{r fig.align = "center", fig.caption="Distribution of `r n.randomization.rr.TAF` (A) estimated number of missing studies according to the trim-and-fill method, and (B) adjusted overall lnRR according to the trim-and-fill method. The dashed line shows the median value."}
# plotting the distribution of the number of missing studies according to the trim-and-fill method
...
```{r fig.align = "center", fig.caption="Distribution of `r n.randomization.smd.TAF` (A) estimated number of missing studies according to the trim-and-fill method, and (B) adjusted overall SMD according to the trim-and-fill method. The dashed line shows the median value."}
# plotting the distribution of the number of missing studies according to the trim-and-fill method
...
```{r fig.align = "center", fig.caption="Distribution of `r n.randomization.rr.3PSM` adjusted overall lnRR according to the 3 parameter selection model. The dashed line shows the median value."}
# plotting the distribution of adjusted overall effect sizes according to the 3 parameter selection model
...
```{r fig.align = "center", fig.caption="Distribution of `r n.randomization.smd.3PSM` adjusted overall SMD according to the 3 parameter selection model. The dashed line shows the median value."}
# plotting the distribution of adjusted overall effect sizes according to the 3 parameter selection model
...
```{r fig.align = "center", fig.caption="Median and 95% quantiles of `r n.randomization.rr.CM` lnRR estimated according to cumulative meta-analysis showing evidence of decline effects."}
...
```{r fig.align = "center", fig.caption="Median and 95% quantiles of `r n.randomization.smd.CM` SMD estimated according to cumulative meta-analysis showing evidence of decline effects."}
