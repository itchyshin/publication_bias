...
Effect sizes with larger uncertainty [larger sqrt(inv_n_tilda)] tend to be larger (i.e. small-study effects) as indicated because the statistically significant slope of the moderator 'sqrt_inv_n_tilda' for lnRR (slope = `r round(estimates.publication.bias.model.rr.srin[2,2],2)`, 95% CI = [`r round(estimates.publication.bias.model.rr.srin[2,3],2)`,`r round(estimates.publication.bias.model.rr.srin[2,4],2)`]; *R<sup>2</sup><sub>marginal</sub>* = `r round(orchaRd::r2_ml(publication.bias.model.rr.srin)[[1]]*100,1)`%) and SMD (slope = `r round(estimates.publication.bias.model.smd.srin[2,2],2)`, 95% CI = [`r round(estimates.publication.bias.model.smd.srin[2,3],2)`,`r round(estimates.publication.bias.model.smd.srin[2,4],2)`]; *R<sup>2</sup><sub>marginal</sub>* = `r round(orchaRd::r2_ml(publication.bias.model.smd.srin)[[1]]*100,1)`%). In S4.2.3.3, we will use all-in publication bias test (multi-moderator meta-regression)see whether the small-study effects exist after accounting for some of the heterogeneity present in the data.   
...
According to this uni-moderator meta-regression, there is an indication of decline effects since the slope of the moderator 'year of publication' is marginally statistically significant for lnRR (slope = `r round(estimates.publication.bias.model.rr.timelag[2,2],2)`, 95% CI = [`r round(estimates.publication.bias.model.rr.timelag[2,3],2)`,`r round(estimates.publication.bias.model.rr.timelag[2,4],2)`]; *R<sup>2</sup><sub>marginal</sub>* = `r round(orchaRd::r2_ml(publication.bias.model.rr.timelag)[[1]]*100,1)`%) and SMD (slope = `r round(estimates.publication.bias.model.smd.timelag[2,2],2)`, 95% CI = [`r round(estimates.publication.bias.model.smd.timelag[2,3],2)`,`r round(estimates.publication.bias.model.smd.timelag[2,4],2)`]; *R<sup>2</sup><sub>marginal</sub>* = `r round(orchaRd::r2_ml(publication.bias.model.smd.timelag)[[1]]*100,1)`%).    
...
```{r fig.height = 5.5, fig.width = 7.5, fig.align = "center", fig.cap="The average effect, as measured by SMD, has marginally declined over the timespan captured by this dataset. The solid line represents the model estimate, the shading shows its 95% confidence intervals."}
# predicting effect sizes for a range of sqrt(inv_n_tilda) from the minimum observed in the data to the maximum observed in data, while year is kept at its mean value, in this case 0 since year was mean-centred
...
##### Adjusting overall effect size estimates for lnRR and SMD

In the main text we provide a two-step approach to calculate the bias-corrected overall effect size estiamte. The rationale is that, when no uncertainty exists (theoretically), we can get the potential true effect from a multiple-moderator multilevel-model (i.e. all-in publication bias test, where the inclusion of moderator variables reduces heterogeneity). We add two more moderators `Experimental.or.Observational.` (2 levels) and `Disturbance_category` (10 levels; we refer to this model as the full model). Note that, if the slope for the moderator 'sqrt_inv_n_tilda' in the all-in publication bias test shown above is not statistically significant, then the best estimate for the adjusted overall effect size would be the *intercept* of that all-in publication bias test. Otherwise, we should use inv_n_tilda instead as moderator (detailed in the main text). 
...
##### Figure 6a-d (redrawn using effective sample size)
...
# prepration
# chaning the pcurve function in dmetar so fonts are large enough to see
